...
import { onUnmounted as onUnmounted2, readonly, ref as ref7, watch as watch3 } from "vue";
import { inBrowser } from "../utils/index.js";
var focusedElement = ref7();
var active = false;
var listeners = 0;
function useFlyout(options) {
  const focus = ref7(false);
  if (inBrowser) {
    if (!active) {
      activateFocusTracking();
    }
    listeners++;
    const unwatch = watch3(focusedElement, (el) => {
...
import { computed as computed14, onMounted as onMounted2, onUnmounted as onUnmounted3 } from "vue";
var lightTint = {
...
import { computed as computed17, onMounted as onMounted3, ref as ref9, watchEffect as watchEffect2 } from "vue";
import { usePageLang as usePageLang3 } from "vuepress/client";
function useLastUpdated() {
  const { theme, page, frontmatter } = useData();
  const lang = usePageLang3();
  const date = computed17(() => page.value.git?.updatedTime ? new Date(page.value.git.updatedTime) : null);
  const isoDatetime = computed17(() => date.value?.toISOString());
  const datetime = ref9("");
  const lastUpdatedText = computed17(() => {
...
import { computed as computed18, toValue as toValue2 } from "vue";
...
import { computed as computed19, ref as ref10, watch as watch4 } from "vue";
...
function useNavbarData() {
  const { theme } = useData();
  return computed19(() => resolveNavbar(theme.value.navbar || []));
}
...
function useNav() {
  const isScreenOpen = ref10(false);
  function openScreen() {
...
import { inject as inject5, onMounted as onMounted4, onUnmounted as onUnmounted4, onUpdated, provide as provide3, ref as ref11 } from "vue";
...
function useActiveAnchor(container, marker) {
  ...
  const routeHash = ref11(router.currentRoute.value.hash);
...
function setupWatermark() {
  ...
  defineWatermarkConfig(computed21(() => {
